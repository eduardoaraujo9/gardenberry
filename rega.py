...
try:
	sql.execute("SELECT ROUND(AVG(temperatura),3) AS media FROM gardenberry.previsao WHERE datahora BETWEEN NOW() AND DATE_ADD(NOW(), INTERVAL IF(HOUR(NOW())<12,12,24) HOUR);")
	r = sql.fetchone()
	media = float(r[0])
	sql.execute("SELECT ROUND(SQRT(SUM(v)/(COUNT(v)-1)),2) AS desvio_padrao FROM ( SELECT temperatura, ROUND((" + str(media) + " -temperatura)*(" + str(media) + " -temperatura),2) AS v FROM gardenberry.previsao WHERE datahora BETWEEN NOW() AND DATE_ADD(NOW(), INTERVAL IF(HOUR(NOW())<12,12,24) HOUR) ) AS dp;")
	r = sql.fetchone()
	...
	r = sql.fetchone()
	column_names = ["precipitacoes","temperaturas","umidades","fut_precipitacoes","fut_temperaturas","rega","hora"]
	r = dict(zip(column_names,r))
	t = 0
	modf = 0
	if int(r["hora"]) < 12:
		print("morning")
		if int(r["rega"]) == 0 and float(r["precipitacoes"]) < 1.6 and float(r["umidades"]) < 80:
			modf = float(1)
	else:
		print("evening")
